    ...
    for k_max in range(num_group_min, num_group_max+1):
        for c in c_set:
            ...
            for k in range(k_min, k_max):
                i = k - k_min
                new_score = score[score < thresholds[-(i + 1)]]
                num_new_score = len(new_score)
                if int(num_piece * c ** i) < num_new_score:
                    thresholds[-(i + 2)] = new_score[-int(num_piece * c ** i)]

            url, score = positive_sample['url'], positive_sample['score']
            ix = [min(np.where(score_s < thresholds)[0]) for score_s in score]
            k = [k_max - i for i in ix]
            for url_s, k_s in zip(url, k):
                bloom_filter.insert(url_s, k_s)
            ...
            test_result = np.zeros(len(url_negative), dtype=int)
            for i, (score_s, url_s) in enumerate(zip(score_negative, url_negative)):
                ix = min(np.where(score_s < thresholds)[0])
                k = k_max - ix
                test_result[i] = bloom_filter.test(url_s, k)

            FP_items = int(sum(test_result)) + len(ML_positive)
            print(f'Number of groups: {k_max}, False positive items: {FP_items}')
